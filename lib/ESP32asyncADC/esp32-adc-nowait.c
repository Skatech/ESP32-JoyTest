...
/*
  ...
  BUT THESE FUNCTIONS ARE VERY USEFUL FOR "FINE TUNNING" AND "NO-WAIT" ADC READING.
  I JUST COPY CODE FROM .c/.h AND TO USE IT IN MY LIBRARY WITHOUT ANY MODIFICATION
...
void __aSetAttenuation(uint8_t attenuation) //adc_attenuation_t
{
...
__attribute__((weak)) bool IRAM_ATTR adcAttachPin(uint8_t pin)
{
...
__attribute__((weak)) bool IRAM_ATTR adcStart(uint8_t pin)
{
...
__attribute__((weak)) bool IRAM_ATTR adcBusy(uint8_t pin)
{
  int8_t channel = digitalPinToAnalogChannel(pin);
  if (channel < 0)
  {
...
__attribute__((weak)) uint16_t IRAM_ATTR adcEnd(uint8_t pin)
{
